...
# === Obstacles & Caching ===
static_obstacles      = set()    # user‐clicked & manually placed obstacles
dynamic_obstacles     = set()    # cleared & rebuilt each frame
ball_positions_cm     = []
...
def compute_best_route(balls_list, goal_name):
    ...
    # combine static + dynamic each frame
    all_obs = static_obstacles | dynamic_obstacles
    ...
    # find best order via brute‐force
    best = None
    best_cost = float('inf')
...
def draw_metric_grid(frame):
    ...
    # draw dynamic obstacles in orange
...
def draw_full_route(frame, ball_positions):
    ...
    for i in range(len(route_cm)-1):
        ...
        path = astar(sc, ec, grid_w, grid_h, get_expanded_obstacles(static_obstacles | dynamic_obstacles))
        if not path:
            continue
        ...
        if pt_cm:
            ...
            if 0 <= gx <= max_gx and 0 <= gy <= max_gy:
                if (gx, gy) in static_obstacles:
                    ...
                    print(f"🟢 Cleared static obstacle at: ({gx}, {gy})")
                else:
                    static_obstacles.add((gx, gy))
                    print(f"🚧 Added static obstacle at: ({gx}, {gy})")
...
def process_frames():
    global ball_positions_cm, dynamic_obstacles, robot_position_cm
    while not stop_event.is_set():
        try:
        ...
        # rebuild dynamic obstacles each frame
        ...
        # 2) DETECT ROBOT & OTHER OBSTACLES (dynamic)
        ...
        for d in preds_v3.get('predictions', []):
            ...
            # sample inside for dynamic obstacle cells
