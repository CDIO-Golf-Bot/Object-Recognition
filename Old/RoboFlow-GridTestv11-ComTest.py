...
def compute_route_from_points(ball_positions, goal_name):
    # 1) Visit all non‐orange balls (greedy by Manhattan)
...
def draw_metric_grid(frame):
    ...
    # Draw obstacles as small red circles
...
def draw_full_route(frame, ball_positions):
    ...
    # We'll accumulate pixel‐space L‐shaped segments (no diagonal)
    overlay = frame.copy()
    path_color = (0, 255, 255)
    total_cm = 0

    # Clear out full_grid_path each time we recompute:
    full_grid_path = []

    # For each consecutive pair in route_cm, run A* on the grid
    for i in range(len(route_cm) - 1):
        ...
        # Append each cell in this segment to full_grid_path
        for cell in path_cells:
            full_grid_path.append(cell)

        # Draw each grid‐step (four‐neighbor) as an axis‐aligned segment in real‐world, projected to pixel
        for j in range(len(path_cells)):
            if j == 0:
                prev_cell = path_cells[0]
            else:
                prev_cell = path_cells[j - 1]
            curr_cell = path_cells[j]

            # Each prev_cell → curr_cell differs by exactly 1 in either x or y
            gx0, gy0 = prev_cell
            gx1, gy1 = curr_cell

            # Convert those two cells to real‐world cm coordinates
            x0_cm, y0_cm = gx0 * grid_spacing_cm, gy0 * grid_spacing_cm
            x1_cm, y1_cm = gx1 * grid_spacing_cm, gy1 * grid_spacing_cm

            # Project both ends to pixel
            ...
            # Draw that single axis‐aligned segment
            cv2.line(
                overlay,
...
def click_to_set_corners(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Right‐click: convert pixel to cm, then to grid cell, and toggle obstacle
...
def send_path(ip: str, port: int, grid_path: list, heading: str):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            # Build JSON payload: { "heading": "N", "path": [ [gx,gy], ... ] }
            payload = {
                "heading": heading,
            ...
            # Prefix length (so robot knows how many bytes to read)
...
def process_frames():
    ...
    while not stop_event.is_set():
        ...
        if homography_matrix is not None:
            ...
            # c) Find all red contours
            contours, _ = cv2.findContours(red_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

            # d) Select the one contour that matches the cross size
            best_contour = None
            best_contour_area = 0

            # Compute approx pixels per cm in x and y
            ...
            for cnt in contours:
                ...
                # Approximate area in cm²
                approx_cm_w = w / px_per_cm_x
                approx_cm_h = h / px_per_cm_y
                approx_cm_area = approx_cm_w * approx_cm_h

                # If bounding‐box too large (> 400 cm²), skip (likely table background)
                if approx_cm_area > 400:
                    continue

                # Among remaining, pick the largest pixel‐area as “the cross”
                if area_px > best_contour_area:
                    best_contour = cnt
                    best_contour_area = area_px

            # e) Sample grid‐cells inside the best_contour
            new_obstacles = set()
            if best_contour is not None:
                # Draw contour in green for debugging
            ...
            # ── HERE’S THE CHANGE ──
            # Instead of “obstacles = new_obstacles”, we merge:
            obstacles |= new_obstacles

        # --- 3) DRAW GRID + ROUTE (A*) AS BEFORE ---
    ...
    while not stop_event.is_set():
        try:
            ...
            if key == ord('q'):
                ...
                # (1) Save the route locally
                if pending_route:
                    save_route_to_file(pending_route)
                else:
                    print("⚠️ No route to save yet.")

                # (2) Send the stored grid cells to the robot
                if full_grid_path:
...
if __name__ == "__main__":
    ...
    # Wait until display thread signals exit
    disp_thread.join()
    cap_thread.join()
    proc_thread.join()
