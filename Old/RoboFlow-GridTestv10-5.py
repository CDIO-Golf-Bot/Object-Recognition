...
def draw_full_route(frame, ball_positions):
    ...
    # Collect all grid cells along the full route (concatenate segments)
    full_cells = []
    for i in range(len(route_cm) - 1):
        start_cell = cm_to_grid_coords(*route_cm[i])
        end_cell   = cm_to_grid_coords(*route_cm[i + 1])
        segment = astar(start_cell, end_cell, grid_w, grid_h, obstacles)
        if segment:
            full_cells.extend(segment)

    # 1) Draw a semi-transparent highlight on each grid cell in path
    for (gx, gy) in full_cells:
        x_cm, y_cm = gx * grid_spacing_cm, gy * grid_spacing_cm
        half_cm = grid_spacing_cm / 2  # half cell
        corners_cm = np.array([
            [[x_cm - half_cm, y_cm - half_cm]],
            [[x_cm + half_cm, y_cm - half_cm]],
            [[x_cm + half_cm, y_cm + half_cm]],
            [[x_cm - half_cm, y_cm + half_cm]]
        ], dtype="float32")

        pts_px = cv2.perspectiveTransform(corners_cm, homography_matrix)
        pts_px = pts_px.reshape(-1, 2).astype(int)
        cv2.fillPoly(overlay, [pts_px], (0, 255, 0))  # bright green fill

    frame_with_cells = cv2.addWeighted(frame, 0.6, overlay, 0.4, 0)

    # 2) Draw arrows between cells but only every other cell to add spacing
    arrow_spacing = 2  # draw an arrow every 2 cells
    for j in range(len(full_cells)):
        if j % arrow_spacing != 0:
            continue
        if j == 0:
            prev_cell = full_cells[0]
        else:
            prev_cell = full_cells[j - 1]
        curr_cell = full_cells[j]
    ...
    # 3) Draw total distance
    cv2.putText(
        frame_with_cells,
        f"Total Path: {total_cm}cm to Goal {selected_goal}",
        (10, frame_with_cells.shape[0] - 20),
...
def process_frames():
    ...
    while not stop_event.is_set():
        ...
        for d in preds.get('predictions', []):
            ...
            # Hard‐coded colors: very light blue for “white” and orange for “orange”
            if "white" in ll:
                color = (200, 200, 255)   # (B, G, R) = light blue
            elif "orange" in ll:
                color = (0, 165, 255)     # standard OpenCV orange
            else:
