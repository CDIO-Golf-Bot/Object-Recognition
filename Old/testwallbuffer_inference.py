...
def create_and_cache_grid_overlay():
    global grid_overlay
    # make a tiny grid in real‐world cm units
    ...
    # now warp that cm-grid into your camera’s pixel frame
...
def click_to_set_corners(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if len(calibration_points) == 4:
            # image pts → real‐world pts
            src_pts = np.array(calibration_points, dtype="float32")                            # pixel coords
            dst_pts = np.array([
                [0, 0],
                [REAL_WIDTH_CM, 0],
                [REAL_WIDTH_CM, REAL_HEIGHT_CM],
                [0, REAL_HEIGHT_CM]
            ], dtype="float32")                                                                 # cm coords

            # H_img2real maps image → real‐world
            H_img2real       = cv2.getPerspectiveTransform(src_pts, dst_pts)
            # H_real2img maps real‐world → image
            H_real2img       = np.linalg.inv(H_img2real)

            inv_homography_matrix = H_img2real
            homography_matrix     = H_real2img
...
def send_path(grid_path: list, heading: str):
    ...
    try:
        ...
        print(f"\U0001f4e8 Sent path → {len(filtered)} points (compressed from {len(grid_path)}), heading={heading}")
    except Exception as e:
...
def process_frames():
    ...
    while not stop_event.is_set():
        ...
        # RUN YOLO INFERENCE
        results    = yolo_model(original, verbose=False)               # single-frame inference
        ...
        for x1, y1, x2, y2, conf, cls_id in detections:
            ...
            # Sort into balls, robot, or obstacles
            if "ball" in lbl:
                ...
                # treat everything else as obstacle seeds
                if homography_matrix is not None and real:
                    new_obstacles.add((gx, gy))

        # Expand and merge new obstacles
        obstacles |= get_expanded_obstacles(new_obstacles)

        # --- RED CROSS DETECTION (unchanged) ---
