...
# Turn parameters
turn_speed_pct = 40    # maximum speed percent
...
# Aux control
def _start_aux(): aux_motor.on(25)
...
def drive_distance(distance_cm, speed_pct=motor_speed, target_angle=None):
...
def perform_turn(angle_deg):
    ...
    _start_aux()
    try:
        # Phase 1: aggressive correction
        while abs(gyro.angle - target_angle) > 3.0:
            error = target_angle - gyro.angle
            power = min(max(abs(error) * 0.4, 10), turn_speed_pct)
            if error > 0:
                tank.on(-power, power)
            else:
                tank.on(power, -power)
        ...
        while abs(gyro.angle - target_angle) > angle_tolerance:
            error = target_angle - gyro.angle
            power = 10  # consistent low power for fine-tune
            if error > 0:
                tank.on(-power, power)
            else:
                tank.on(power, -power)
        ...
        tank.off()
        _stop_aux()
        print("Final heading: {:.2f} deg (target was {:.2f})".format(gyro.angle, target_angle))
